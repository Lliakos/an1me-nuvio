...
// SLUG_MAP: TMDB ID -> slug or per-season object
// Per-season: { '1': slug, '2': slug | { slug, splitAfter, next: { slug, offset } }, '*': fallback }
var SLUG_MAP = {
...
function resolveSlugAndEp(targetId, season, episode) {
  ...
  if (
    seasonEntry.splitAfter &&
    ep > seasonEntry.splitAfter &&
    seasonEntry.next
  ) {
    var adj = ep - seasonEntry.next.offset;
    remoteLog(
      "[An1me] Split: ep " + ep + " -> " + seasonEntry.next.slug + " ep " + adj,
    );
    return { slug: seasonEntry.next.slug, episode: adj };
  }
...
function processEmbed(data) {
  try {
    ...
    var isGoogle =
      directUrl.indexOf("photos.google.com") !== -1 ||
...
// Search an1me.to for a slug matching the given title.
// Tries the search page and matches hrefs against title keywords.
  ...
  return plainFetch(searchUrl, AN1ME_HEADERS)
    .then(function (html) {
      ...
      while ((m = hrefRe.exec(html)) !== null && !found) {
        ...
        var watchMatch = clean.match(
          /^\/watch\/([a-z0-9\-]+)-episode-\d+\/?$/i,
        );
        var candidate = animeMatch
          ? animeMatch[1]
          : watchMatch
            ? watchMatch[1]
            : null;
...
// Try multiple title variants to find the slug:
// 1. English title (e.g. "Frieren: Beyond Journey's End")
// 2. Original/Japanese title transliterated (e.g. "Sousou no Frieren")
// 3. Cleaned English title (drop subtitle after colon/dash)
...
function searchAnimeSlug(info) {
  ...
  // Also try cleaned version (drop "Season N", ": subtitle", etc.)
  var cleaned = english
    .replace(/:\s*.+$/, "")
    .replace(/(Season|Part)\s*\d+/gi, "")
    .trim();
  if (cleaned && cleaned !== english && queries.indexOf(cleaned) === -1)
    queries.push(cleaned);

  function tryNext(i) {
    if (i >= queries.length) {
      // Last resort: slugify the best title we have
...
function getStreams(tmdbId, mediaType, season, episode) {
  ...
  // Dict miss — fetch TMDB metadata then search
